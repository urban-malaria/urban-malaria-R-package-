...
rank <- function(composite_score_data) {
  ...
  urban_data <- composite_score_data %>% dplyr::select(WardName, Urban.x, WardCode.x)

  # reshape the data to long format, keeping only model score columns and ward names
  melted_data <- composite_score_data %>%
    dplyr::select(WardName, WardCode.x, starts_with("model_")) %>%  # select model columns and ward name
    reshape2::melt(id.vars = c("WardName", "WardCode.x"), variable.name = "variable", value.name = "value")

  # compute an overall composite score for each ward (e.g., mean of all model scores)
  ward_scores <- melted_data %>%
    group_by(WardName, WardCode.x) %>%
  ...
  plottingdata <- ward_scores %>%
    left_join(urban_data, by = c("WardName", "WardCode.x")) %>%
    mutate(flag_not_ideal = ifelse(Urban.x == "No" & rank <= 5, TRUE, FALSE))
